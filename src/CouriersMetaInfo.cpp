...
namespace {
class CouriersMetaInfo final : public userver::server::handlers::HttpHandlerJsonBase {
 public:
  static constexpr std::string_view kName = "handler-couriers-meta-info";

  CouriersMetaInfo(const userver::components::ComponentConfig& config,
  ...
  userver::formats::json::Value HandleRequestJsonThrow(
    ...
    auto start_date = userver::utils::datetime::Date(userver::utils::datetime::DateFromRFC3339String(request.GetArg("startDate")));
    auto end_date = userver::utils::datetime::Date(userver::utils::datetime::DateFromRFC3339String(request.GetArg("endDate")));

    auto start_date_point = start_date.GetSysDays();
    auto end_date_point = end_date.GetSysDays();

    // const std::time_t t_c = std::chrono::system_clock::to_time_t(start_date.GetSysDays());
    // LOG_INFO() << "parsed start and end dates like " << userver::utils::datetime::ToString(start_date) << " "<< userver::utils::datetime::ToString(end_date);
    // LOG_INFO() << "parsed start and end dates in chrono style like " << std::put_time(std::localtime(&t_c), "%F %T.\n");
    
    auto courier_id = std::stoi(request.GetPathArg("courier_id"));
    auto result_earnings = pg_cluster_->Execute(
        userver::storages::postgres::ClusterHostType::kMaster,
        "SELECT SUM(ord.cost) AS cost, COUNT(*) AS cnt FROM bds_schema.completed_orders AS hist "
        "JOIN bds_schema.orders AS ord ON (hist.order_id=ord.order_id) "
        "WHERE hist.courier_id = $1 AND hist.complete_date >= $2 AND hist.complete_date < $3 ",
    ...
    if (hours.count() <= 0) {
      ...
      throw userver::server::handlers::ClientError(
            userver::server::handlers::ExternalBody{
                fmt::format("Incorrect start adn end dates {}, {}", userver::utils::datetime::ToString(start_date), userver::utils::datetime::ToString(end_date))});
    }
